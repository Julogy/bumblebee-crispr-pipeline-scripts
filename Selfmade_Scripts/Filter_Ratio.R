...
ratio_cutoff <- as.numeric(getArg("-ratio"))     # z.B. 0.2 statt fixer Distanz
prob_cutoff <- as.numeric(getArg("-prob"))
max_repeats <- as.numeric(getArgOptional("-max_repeats", Inf))

Vorlage <- read.table(input_file, stringsAsFactors=FALSE)
...
#------------ Filtern ------------
dist_values <- classify_sorted[,10]     # Achtung: Spaltenindex verschiebt sich um 1, da "Group" vorne eingefügt wurde -> prüfen!
prob_values <- classify_sorted[,4]

seq_length  <- nchar(classify_sorted$Repeat)
ratio_values <- dist_values / seq_length

Dist_Greater_X <- classify_sorted[
  (!is.na(ratio_values) & ratio_values < ratio_cutoff) |
...
out_filtered <- file.path(dirname(input_file),
                          paste0("Ratio_", ratio_cutoff, "_filtered_and_classified_repeats.csv"))
write.csv(Dist_Greater_X, out_filtered, row.names = FALSE)